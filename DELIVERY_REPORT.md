...
## الملحق الأربعون — "Hotfix 9.1.0: التحقق الفعلي من SQL في CI + إصلاحا Phase 9 المكتشفان به" (ترحيلتان 0231–0232)

الملحقان 38 و39 سجّلا بوضوح أن اختبارات SQL/التزامن/الترقية الخاصة بـPhase 9
**مكتوبة بالكامل لكن لم تُشغَّل فعليًا** لعدم توفر خادم Postgres في بيئة
التنفيذ، ووصفا ذلك بأنه «فجوة حقيقية مُصرَّح بها لا مُغطّاة». هذا الملحق يغلق
تلك الفجوة: شُغّلت الاختبارات فعليًا مقابل PostgreSQL 16 حقيقي، فكشفت **عيبين
حقيقيين في منتج Phase 9** لم يكن ممكنًا اكتشافهما بأي فحص ثابت.

**السبب الجذري للفجوة:** وظيفة CI الوحيدة (`verify`) كانت تشغّل طبقة
TypeScript فقط (Vitest/tsc/ESLint/build). لم يكن أي ملف من
`supabase/migrations/*.sql` أو `supabase/tests/*.test.sql` يُنفَّذ في CI إطلاقًا،
ولهذا مرّت 489/489 خضراء بينما كانت دالتان في Phase 9 معطّلتين تمامًا.

### 1) الترحيلتان الجديدتان (0231–0232)

| # | الترحيلة | العيب المُصلَح |
|---|---|---|
| 0231 | `inventory_stock_movements_select_policy_fix.sql` | سياسة `inventory_stock_movements_select` (0228) كانت تستدعي `user_visible_store_ids(auth.uid())` — وهي مُصرَّح بها لـ`service_role` وحده (0017 يسحبها من PUBLIC عمدًا لأن معاملها uuid يتيح استكشاف نطاق مستخدم آخر). سياسة RLS تُقيَّم بدور المُستعلِم، فكان **كل `SELECT` مباشر على الجدول من جلسة `authenticated` حقيقية يُجهَض** بـ`permission denied for function`، مهما كانت صلاحيات المستخدم. الإصلاح: استخدام الغلاف الذاتي `my_visible_store_ids()` (0017)، بنفس دلالات النطاق حرفيًا (`select * from user_visible_store_ids(auth.uid())`) — يتغيّر فقط كون الدالة قابلة للاستدعاء. **الترحيلة 0059 تحمل تحذيرًا حرفيًا من هذا الخطأ نفسه (`daily_closings_select`)، و0228 أعادت إدخاله.** |
| 0232 | `inventory_update_item_ambiguous_column_fix.sql` | `update_inventory_item()` (0229) مُعرَّفة `returns table (id uuid, row_version bigint)`، ما يضع معاملَي إخراج بنفس الاسمين في نطاق الدالة؛ ثم أشار جسمها إلى `id` و`row_version` **بلا تأهيل** مقابل `public.inventory_items`. plpgsql يحسم هذا التصادم وقت **التنفيذ** لا وقت الإنشاء، فطُبِّقت 0229 بنظافة تامة بينما كانت الدالة تفشل في **كل استدعاء** بـ`column reference "id" is ambiguous` — أي غير عاملة 100% كما سُلِّمت. الإصلاح: تأهيل الجداول بأسماء مستعارة (`i`/`ii`). |

**لم تُعدَّل أي ترحيلة من 0001 إلى 0230، ولا `supabase/seed.sql`** — الإصلاحان
إضافيان بالكامل، على نمط 0230 نفسه.

### 2) منع `row_version` بقيمة NULL — داخل منطق المنتج

ضمن 0232 أيضًا: كان فحص التزامن التفاؤلي `v_old.row_version <>
p_expected_version`. مقابل `NULL` تكون النتيجة `NULL` لا `TRUE`، فمتصل يُغفل
الإصدار كان **يتخطى الفحص كليًا** ويكتب فوق ما التزمت به جلسة أخرى. صار NULL
مرفوضًا صراحة، والمقارنة نفسها `is distinct from` (آمنة مقابل NULL).

أُثبِت أن الحارس مُغطّى فعليًا عبر **اختبار طفرة**: بإزالة الحارس من **منطق
المنتج وحده** (وملف الاختبار بلا أي تعديل) يفشل
`inventory_core_phase9.test.sql` برسالة `a NULL expected row_version bypassed
the optimistic-concurrency check`، ويعود للنجاح بإعادة تطبيق 0232 — أي أنه
اختبار حقيقي لا تأكيد مكتوب حول سلوك ناجح أصلًا. **نطاق مقصود:** نمط `<>`
نفسه قائم في دوال تحديث أقدم (0075/0086/…) لكنها ترحيلات مجمَّدة خارج نطاق
Phase 9، فلم تُمس.

### 3) اختبار التزامن الحقيقي — نتيجة تشغيل فعلية

`supabase/tests/inventory_core_phase9_concurrency.test.sql` شُغِّل فعليًا
بجلستين حقيقيتين عبر `dblink`: رصيد ابتدائي 10، محاولتا سحب −6 متزامنتان.
المُثبَت: B **حُجبت فعليًا** طالما بقيت معاملة A مفتوحة تحمل القفل الاستشاري،
ثم بعد التزام A أُعيد تقييمها على الرصيد الفعلي (4) لا على قراءة قديمة (10)
فرُفضت؛ والتحقق النهائي عبر `SUM()` حيّ: الرصيد **4** وعدد الحركات **2**
بالضبط — لا صف لمحاولة B المرفوضة.

### 4) وظيفة PostgreSQL الجديدة في CI

أُضيفت وظيفة `sql` إلى `.github/workflows/ci.yml`: خدمة `postgres:16`، فحص
توفر `dblink`، إنشاء أدوار `anon`/`authenticated`/`service_role`، ثم بناء قاعدة
**template** واحدة (harness + كل الترحيلات + `seed.sql`) تُستنسخ لكل اختبار عبر
`CREATE DATABASE ... TEMPLATE` — عزل حقيقي بلا كلفة إعادة بناء 232 ترحيلة لكل
ملف. تشغّل الوظيفة **كل** اختبارات SQL بأنواعها الثلاثة (عادية، تزامن بجلستين
عبر dblink، ترقية عبر سكربتاتها التي تبني خط الأساس التاريخي الخاص بها).
**لا `continue-on-error` ولا تخطٍّ لأي ملف.**

`POSTGRES_PASSWORD` حاملة وظيفة هنا: عدة ملفات تزامن تستدعي `dblink_connect`
بعد `set local role authenticated`، وdblink يرفض اتصال غير-superuser ما لم
تُستخدم كلمة مرور فعليًا — خادم بمصادقة `trust` يفشلها بـ`password or GSSAPI
delegated credentials required` رغم تمرير كلمة المرور.

### 5) إصلاح افتراض «أغسطس 2026» في اختبارَي السيناريو الذهبي

`reports_dashboard_golden_scenario.test.sql` و`reports_detail_golden_scenario.
test.sql` كانا يفترضان أن انعكاسات التثبيتة تقع في **أغسطس 2026**. لكن التثبيتة
تُمرِّر `public.business_today()` صراحةً لدوال العكس/الإلغاء، فمن سبتمبر 2026
فصاعدًا انتقلت الانعكاسات إلى الشهر الجاري وفشل الاختباران
(`expected +555.00, got 0`). نافذة الانعكاس صارت **مُشتقّة** من
`business_today()` نفسها، فتتبع الساعة الحقيقية عبر حدود الشهر **والسنة**. لم
تُضعَّف أي assertion — نفس الأرقام تمامًا، مقابل النافذة التي تنتمي إليها
فعلًا — بل أُضيفت assertions أقوى: كل شهر يقع **بين** الأحداث الأصلية
وانعكاساتها يجب أن يساوي صفرًا تمامًا، والتقرير السنوي صار يُثبت الحالتين
(انعكاسات في 2026 → 565.00 في سنة واحدة؛ انعكاسات في سنة لاحقة → 68.00 لسنة
الأصل و497.00 لسنة الانعكاس). أُثبِت ذلك بتشغيل الاختبارين مقابل ساعة مُحاكاة
في 2027، فنجحا وشُغِّل فرع تدحرج السنة و8 أشهر فاصلة صفرية.

### 6) إصلاح تقطّع `settlements_phase7_concurrency.test.sql` — إصلاح حتمي داخل الاختبار

كان القسم B يفشل **3 مرات من 6 تشغيلات**. السبب الجذري: المزامنة كانت مبنية
على `dblink_is_busy()`، وهي لا تقول سوى «هذا الاتصال لم يُرجع نتيجة بعد» — لا
تفرّق بين «الجلسة الثانية محجوبة فعلًا على قفل الأولى» و«لم تبدأ بعد» أو «ما
تزال تعمل عملًا عاديًا». فكان سيناريو يزعم إثبات **تنافس** قد يمر بينما
الجلستان نُفِّذتا تسلسليًا بلا أي تداخل.

**الإصلاح داخل الاختبار وحده — لا مساس بمنطق المنتج** (وقد ثبت أن العيب ليس
فيه: الفهرس الفريد الجزئي وحارس المطالبات يعملان، انظر اختبار الطفرة أدناه):

1. استدعاء A صار **متزامنًا** (`dblink()` بدل `dblink_send_query()`): عند
   عودته يكون INSERT الخاص بـA قد نُفِّذ فعلًا يقينًا، ومعاملته ما تزال مفتوحة.
   الإرسال غير المتزامن القديم كان قد يعود قبل تنفيذ الاستعلام أصلًا.
2. `_p7cc_wait_blocked_by(pid, blocker)` جديدة مبنية على **`pg_blocking_pids()`**
   — تنتظر حتى تُرصد جلسة B **محجوبة على backend جلسة A تحديدًا**. هذا الشرط
   الواحد يُثبت نصفَي الادعاء معًا: أن المعاملتين تداخلتا زمنيًا، وأن B منتظرة
   خلف قفل A — **قبل** تحرير A. وهو ما لم يكن `dblink_is_busy()` ليُظهره أبدًا.
3. التأكيد النهائي صار على B مباشرة (`assert v_b_failed`) بدل «إحدى المحاولتين
   فشلت» — أقوى، ولا يمكن إرضاؤه بفشل A لسبب غير متصل.

الانتظار محدود بميزانية زمنية ويفشل بصوت عالٍ إن لم تتحقق الحالة — **ليس**
إعادة محاولة حتى النجاح، والعملية تحت الاختبار لا تُعاد أبدًا، ولا اعتماد على
`sleep` وحده.

**النتيجة: 20/20 تشغيلًا متتاليًا ناجحًا**، كلٌّ على قاعدة نظيفة.

**إثبات أن النجاح ليس شكليًا (طفرتان مستقلتان):**

| الطفرة | النتيجة |
|---|---|
| حذف حارس التعارض `settlement_source_claims_active_unique_idx` | **فشل** — B لم تُرصد محجوبة على قفل A |
| كسر التزامن: التزام A **قبل** إرسال B (تسلسل بلا تداخل) | **فشل** — B لم تُرصد محجوبة على قفل A |

الطفرة الثانية هي الدليل الحاسم: **النسخة القديمة كانت ستمر بها** (لأن B تفشل
عند الفحص المسبق فيتحقق شرط «إحدى المحاولتين فشلت»)، بينما الجديدة ترفضها
لأنها تشترط تداخلًا حقيقيًا مُثبَتًا.

### 7) الاختبار اليتيم + فحص التغطية في CI

`supabase/tests/upgrade_phase8_reports_dashboard.test.sql` كان الملف الوحيد في
المستودع بلا أي runner في `scripts/` — لم يكن يُنفَّذ في أي مكان طوال عمره ولم
يلاحظ أحد. خط الأساس مأخوذ من **ترويسة الملف نفسه حرفيًا** لا اعتباطًا:
«Upgrade-safety test for 0199-0204 on top of the FROZEN 0001-0198 baseline
(§0)». فأُنشئ `scripts/run_upgrade_test_phase8_reports_dashboard.sh` بخط أساس
**198**، و**بلا أي pre-fixture** — وهذا مقصود ومنصوص عليه في الملف نفسه («No
`\i` of the Phase 8 golden fixture here BY DESIGN»): الغرض إثبات أن الـ21 RPC
آمنة مقابل قاعدة تحمل بيانات `seed.sql` المرجعية فقط ولم تشغّل تثبيتة Phase 8
إطلاقًا. النتيجة عند التشغيل الفعلي: **PASSED** (19 تأكيدًا).

وأُضيفت خطوة **SQL test coverage** في CI (فحص ملفات خالص، يعمل أولًا ويفشل
سريعًا) ترفض وجود أي `*.test.sql` بلا مسار تنفيذ واضح. التعامل مع الاختبارات
السلبية المقصودة صحيح: `run_upgrade_test_hotfix_7_1_3_broken_fixture.sh` يُثبت
أن ترحيلة 0197 **يجب أن تُجهض** ولا يشغّل ملف اختبار أصلًا، فهو سكربت لا ملف
اختبار يتيم — ولا يُشار إليه بالفحص، بينما يُنفَّذ ضمن حلقة الترقية كبقية
الـrunners.

### 8) النتائج الفعلية — استبدالًا للعبارة القديمة

تشغيلان كاملان من قواعد نظيفة (كل اختبار على نسخة نظيفة من قاعدة template):

| النوع | العدد | النتيجة |
|---|---|---|
| اختبارات SQL العادية | 28 | **28/28** في كلا التشغيلين |
| اختبارات التزامن (dblink، جلستان حقيقيتان) | 6 | **6/6** في كلا التشغيلين |
| اختبارات الترقية (runners، منها الجديد) | 14 | **14/14** في كلا التشغيلين |
| `settlements_phase7_concurrency` وحده | 20 تشغيلًا | **20/20** |
| Vitest / tsc / ESLint / build | — | 489/489 · صفر أخطاء · صفر أخطاء · 51/51 صفحة |

**GitHub CI (مَنقول عن تحقُّق مالك المستودع من واجهة GitHub — لم يتحقق منه
الوكيل، إذ لا `gh` ولا رمز وصول في بيئته):** Run #14 على `ee4a5af` **نجح**
خلال 1m32s، وRun #15 الخاص بالـPR **نجح** خلال 2m4s، وRun #16 بعد دمج Phase 9
إلى `main` **نجح**.

**الملفات:** ترحيلتان جديدتان (0231، 0232)؛ `.github/workflows/ci.yml` (وظيفة
`sql` + خطوة فحص التغطية)؛ `scripts/run_upgrade_test_phase8_reports_dashboard.sh`
(runner جديد للاختبار اليتيم)؛ `supabase/tests/settlements_phase7_concurrency.
test.sql` (مزامنة حتمية عبر `pg_blocking_pids()`)؛
`supabase/tests/inventory_core_phase9.test.sql`،
`inventory_core_phase9_concurrency.test.sql`، `upgrade_phase9_inventory.test.sql`
(فجوات تثبيتات: `stores.create`، `audit_logs.view`، تحديث نطاق فرع كان يفشل
بصمت تحت RLS، ودقّة سُلَّم `numeric(12,3)`)؛ `reports_dashboard_golden_scenario.
test.sql`، `reports_detail_golden_scenario.test.sql`؛
`tests/inventory-money-string-invariant.test.ts` (توافق CRLF/LF)؛
`TEST_RESULTS_PHASE_9_INVENTORY.md` و`README.md` جديدان/محدَّثان.

**خلاصة الملحق الأربعين:** أُغلقت الفجوة التي صرّح بها الملحقان 38 و39 —
اختبارات SQL تُشغَّل الآن فعليًا في CI مقابل PostgreSQL 16، بأنواعها الثلاثة
وبعزل حقيقي. التشغيل الفعلي نفسه كشف عيبين في Phase 9 كانا سيصلان الإنتاج
(سياسة RLS معطِّلة لكل قراءة مباشرة، ودالة تحديث غير عاملة إطلاقًا)، وأُصلحا
إضافيًا في 0231/0232 دون مساس بأي ترحيلة من 0001–0230 ولا بـ`seed.sql`.

---

## الملحق الحادي والأربعون — "Phase 10: نواة مصروفات الفروع (Store Expenses Core)" (ترحيلات 0233–0236)

بعد موافقة صريحة من المستخدم على النطاق الموصى به في تقرير استكشاف Phase 10،
نُفِّذت **Phase 10 = نواة مصروفات الفروع فقط**: كتالوج تصنيفات عام + دفتر
مصروفات إضافي-فقط مُقيَّد بالفرع وبتاريخ العمل، عمليات يدوية بالكامل
(تسجيل/عكس مؤرَّخ)، وبلا أي وحدة من الوحدات المؤجَّلة (Purchasing/موردين،
Attachments، Payroll، دورات اعتماد، GL/COGS، Bank/Carrier/Salla، وأي ربط
تلقائي بالمبيعات أو المخزون)، وبلا محاسبة ضريبة مدخلات قابلة للاسترداد.

**الفجوة التي أُغلقت:** لم يكن للنظام أي مفهوم لمصروف تشغيلي — لا جدول ولا
RPC ولا صلاحية ولا شاشة. ونتيجةً لذلك كانت صيغة `net_operating_return` (0200)
تجمع ربح المبيعات والشحن والتعديلات **بلا أي حدّ للمصروفات**، أي أنها مساهمة
تشغيلية قبل الإيجار والرواتب والكهرباء.

### 1) الترحيلات الجديدة (0233–0236)

| # | الترحيلة | تُضيف |
|---|---|---|
| 0233 | `store_expenses_permissions.sql` | خمس صلاحيات (`expenses.view`/`create`/`reverse`/`manage_categories`/`process_closed_day`) ومنحها للأدوار. **مزروعة في الترحيلة نفسها لا في `seed.sql`** — ترقية الإنتاج لا تعيد تشغيل seed أبدًا. توثيق صريح لسبب **عدم** تسجيل نطاق قفل استشاري جديد. |
| 0234 | `store_expenses_schema.sql` | `expense_categories` (كتالوج عام، دورة حياة نشط/معطّل، `row_version`، رمز دائم فريد بلا حساسية لحالة الأحرف) و`store_expenses` (دفتر إضافي-فقط مُقيَّد بالفرع، `amount numeric(14,2)` موقَّع، بلا أي عمود إجمالي — الإجمالي دائمًا `SUM()` حيّ)؛ triggers ترفض أي `UPDATE`/`DELETE`؛ فهرس فريد جزئي يضمن عكسًا واحدًا لكل مصروف؛ سياسات RLS طبقة-A. |
| 0235 | `store_expenses_rpcs.sql` | ثماني دوال `SECURITY DEFINER` بـ`search_path` مثبَّت: إنشاء/تعديل/تفعيل/تعطيل تصنيف، `record_store_expense()`، `reverse_store_expense()`، وقراءتان مُقسَّمتان صفحيًا. كل قيمة مالية تُعاد `::text` **مقروءة من الصف المُدرَج** لا من معامل المتصل. |
| 0236 | `dashboard_expenses_reporting.sql` | `get_dashboard_summary_with_expenses()` — غلاف إضافي بالكامل، ومُجمِّع داخلي `_store_expenses_total_for_scope()`. |

**لم تُعدَّل أي ترحيلة من 0001 إلى 0232، ولا `supabase/seed.sql`.**

### 2) نموذج الدفتر — لماذا جدول واحد بمبلغ موقَّع

التصحيح ليس `UPDATE` ولا `DELETE` أبدًا، بل **صف جديد** بـ`entry_kind='reversal'`
ومبلغ **سالب** و**تاريخ عمل خاص به** (عقد §85 Event Date) — فلا يتغيّر رقم فترة
مقفلة بأثر رجعي. وإبقاء النوعين في دفتر واحد موقَّع (نمط `inventory_stock_movements`
0228) يجعل إجمالي أي فترة `SUM(amount)` حرفيًا بلا join وبلا تجميع ثانٍ، ويجعل
حركة العكس تُصافي أصلها تلقائيًا في أي فترة تضمّهما — **بلا طرح مزدوج**.

### 3) الصلاحيات

`super_admin` و`admin`: الخمس. `supervisor`: الأربع التشغيلية **دون**
`manage_categories` — يطابق حرفيًا 0133 التي تمنح supervisor
`adjustments.reverse`/`process_closed_day` وتحجب عنه `adjustments.manage_types`.
`accountant`: **عرض فقط** — يطابق 0227 (`inventory.view`) و0133.

### 4) صيغ التقارير — قبل وبعد المصروفات

**لم تُمسّ** `get_dashboard_summary()` ولا `get_dashboard_summary_with_comparison()`
بسطر واحد، فبقيت الصيغة القديمة ودلالتها وقيمها التاريخية كما هي:

```
net_operating_return = effective_net_sales_profit + net_shipping_result + net_adjustments_result
```

والجديد **غلاف إضافي** يستدعي الدالة الكانونية ويضيف فقط:

```
operating_contribution_before_expenses = net_operating_return   (نسخة jsonb حرفية، بلا إعادة تشكيل)
operating_expenses_total               = SUM(amount) على الدفتر الموقَّع
net_operating_result_after_expenses    = operating_contribution_before_expenses − operating_expenses_total
```

كلا المجموعتين تحترم §79: من لا يملك `expenses.view` **لا تصله المفاتيح
إطلاقًا** (غياب حقيقي لا أصفار)، ومن لا يملك الصلاحيات المالية لا يكتسب
`net_operating_return` عبر هذا الغلاف.

**السيناريو الذهبي يُثبت القيمتين مستقلتين على نفس التثبيتة:** مع مصروف حقيقي
**18.00** في يوليو، تبقى `net_operating_return` = **68.00** بالضبط (وكذلك عبر
غلاف المقارنة)، وتُصبح النتيجة بعد المصروفات **50.00**.

**تسمية الواجهة:** عنوان البطاقة صار «المساهمة التشغيلية قبل المصروفات» —
القيمة ومعناها لم يتغيّرا (صيغتها لم تحوِ حدّ مصروفات قط)، لكنها لم تعد تُقرأ
كنتيجة بعد المصروفات.

### 5) الصفحات والصلاحيات في الواجهة

`/expenses` (فلاتر بالتاريخ/الفرع/التصنيف/النوع/البحث، بطاقات إجمالي/عكوسات/صافي،
تسجيل، عكس مؤرَّخ، تصدير PDF/Excel) و`/master-data/expense-categories` (إنشاء،
تعديل بتزامن تفاؤلي، تفعيل/تعطيل). عنصر تنقل جديد ("المصروفات") مقيَّد بـ
`expenses.view`، ومدخل في صفحة البيانات الأساسية. تقرير `expenses` في سجل
التصدير **يعيد استخدام نفس محرك `list_store_expenses()`** الذي تستخدمه الشاشة
(§39 محرك تقارير واحد، بلا استعلام تصدير موازٍ).

### 6) قرارات تصميم أساسية

1. **لا نطاق قفل استشاري جديد:** المصروف إلحاق غير مشروط بلا رصيد مشتق
   يُسلسَل؛ سباق الإغلاق اليومي يغطيه الزوج القائم على المفتاح 1002؛ و«عكس
   واحد كحد أقصى» فهرس فريد جزئي — ضمان أقوى من قفل لأنه يصمد حتى لو نسي
   متصل مستقبلي أخذ القفل.
2. **العكس يشتق كل شيء من الأصل:** الدالة لا تأخذ معامل مبلغ ولا فرع ولا
   تصنيف إطلاقًا — تُقرأ من الصف الأصلي المقفول بـ`for update`.
3. **لقطة اسم التصنيف** داخل كل قيد: إعادة تسمية تصنيف لا تُشوِّه مصروفًا
   تاريخيًا، والتصنيف يُعطَّل ولا يُحذف.
4. **الكتابة بنطاق الفروع القابلة للتشغيل، والقراءة بالمرئية** — وأي فلتر فرع
   خارج النطاق يُرفض لا يُضيَّق صامتًا (§8).

### 7) الاختبارات — نُفِّذت فعليًا مقابل PostgreSQL 16.4 حقيقي

| النوع | الملف | النتيجة |
|---|---|---|
| SQL أساسي | `store_expenses_phase10.test.sql` | **PASSED** — حدود الصلاحيات، منع القيم غير الصالحة، نطاق الفروع، الإلحاق-فقط (مُختبَر بـsuperuser يتجاوز RLS)، دورة حياة التصنيف، التزامن التفاؤلي بما فيه رفض NULL، العكس ومشتقاته، الإغلاق اليومي، التدقيق، والتقارير |
| تزامن حقيقي | `store_expenses_phase10_concurrency.test.sql` | **PASSED — 10/10 تشغيلات متتالية** على قواعد نظيفة معزولة |
| ترقية | `upgrade_phase10_store_expenses.test.sql` + runner | **PASSED** من خط الأساس المجمَّد 0232 |
| Vitest | 3 ملفات جديدة + تغطية التصدير | ضمن **526/526** |

اختبار التزامن يغطي **ثوابت تزامن حقيقية فقط** (لم يُختلق سيناريو شكلي): سباق
العكس المزدوج، الإغلاق اليومي (EXCLUSIVE) مقابل التسجيل (SHARED)، وتفرّد رقم
المصروف. كلٌّ يُثبت التداخل الفعلي عبر `pg_blocking_pids()` قبل تحرير الجلسة
الأولى.

**خلاصة الملحق الحادي والأربعين:** دفتر مصروفات تشغيلية جديد كليًا ومستقل تمامًا
عن المبيعات والمخزون — كتالوج عام، دفتر إضافي-فقط موقَّع بتاريخ عمل ومُقيَّد
بالفرع، تصحيح بالعكس المؤرَّخ حصرًا، وطبقة تقارير **تحافظ على
`net_operating_return` حرفيًا** وتضيف بجانبه صافي النتيجة بعد المصروفات. لا
تعديل على أي ترحيلة من 0001–0232 ولا على `seed.sql`، ولا أي وحدة خارج النطاق
المعتمد. **نتائج CI على GitHub غير متاحة بعد — لم يُدفع الفرع.**

---

## الملحق الثاني والأربعون — "Hotfix 10.1.0: إكمال واجهة ضريبة القيمة المضافة + إزالة حالة قريبًا القديمة للشحنات" (بلا ترحيلات جديدة)

إصلاح واجهة بحت **بلا أي ترحيلة جديدة وبلا أي تعديل على قاعدة البيانات** —
يبني الواجهة الناقصة فوق خلفية كانت مكتملة أصلًا منذ الترحيلة 0058.

### 1) الفجوة — صلاحيات وRPCs بلا واجهة

`vat_rate_versions` (0058، مُقوّاة في 0066 لتأخذ قفل البيانات المالية الحصري)
موجودة منذ Phase 3، ومعها `create_vat_rate_version()` و`cancel_vat_rate_version()`
و`vat_rate_for_date()`/`_safe()`، والصلاحيتان `vat_rates.view`/`vat_rates.manage`
**مزروعتان وممنوحتان لأربعة أدوار** في `seed.sql`، وتسميتا التدقيق
`vat_rate_version.create/update` موجودتان في `action-labels.ts`.

ومع ذلك لم يكن في `src` **أي** صفحة أو استعلام أو server action أو استدعاء
`.rpc()` يلمس أيًّا منها — الشقيقتان (`manufacturing_fee_versions` و
`payment_method_fee_versions`) لكلٍّ صفحتها، وVAT وحدها كانت غائبة عن
`master-data/page.tsx`. **الأثر العملي:** لا يمكن تغيير نسبة الضريبة إلا
بـSQL مباشر، والصلاحيتان الممنوحتان لا تحرسان شيئًا، وتسميتا التدقيق تنتظران
أحداثًا يستحيل على التطبيق إنتاجها.

### 2) ما أُضيف

`/master-data/vat-rates` — صفحة مقيَّدة بـ`vat_rates.view` للقراءة، وكل عنصر
كتابة فيها مقيَّد إضافيًا بـ`vat_rates.manage` (وتُعيد الـRPCs فرضه على أي
حال). تعرض: النسبة السارية حاليًا، الإصدار القادم إن وُجد مع إمكانية إلغائه،
والسجل التاريخي الكامل بما فيه الإصدارات الملغاة.

بنية الملفات تطابق `src/features/manufacturing-fees/**` حرفيًا: `schema.ts`
(Zod، النسبة تبقى **نصًا** حتى الـRPC بلا `Number()`/`parseFloat()`، بسقف 3
منازل عشرية مطابق لـ`numeric(6,3)`)، `queries.ts` (فصل «الساري» عن «القادم»
بنفس منطق `vat_rate_for_date()` — لا يُقدَّم إصدار مستقبلي على أنه ساري، وهو
نفس درس Financial Integrity Patch 2.1 بند 8)، `actions.ts` (يستدعي الـRPCs
القائمة فقط)، ومكوّنان للحوار والإلغاء.

**لم تُعدَّل أي معادلة ضريبة، ولا أي ترحيلة، ولا `supabase/seed.sql`.**
التدقيق يبقى كما هو عبر مُشغِّل `vat_rate_versions` القائم.

### 3) إزالة حالة "قريبًا" القديمة للشحنات

`nav-items.ts` كانت تضع `comingSoon: true` على «الشحنات» رغم اكتمال الوحدة منذ
Phase 5 — تحقّقت من ذلك قبل الإزالة: 3 مسارات، 9 ملفات ميزة، 24 ترحيلة، 4
مجموعات اختبار SQL (منها اختبار تزامن `dblink` حقيقي)، ومدخل تقرير/تصدير
كامل. فكانت القائمة الجانبية تصف وحدة عاملة بأنها «قريبًا». أُزيلت الراية
فقط؛ **لم يُمَس أي منطق أعمال للشحن**، وأُبقيت الآلية نفسها متاحة لوحدة
مستقبلية غير مبنية فعلًا.

### 4) الاختبارات والتحقق

ملف Vitest جديد `tests/vat-rates-actions-and-navigation.test.ts` (17 اختبارًا):
حدود `vat_rates.manage` مقابل `vat_rates.view`، إعادة استخدام الـRPCs القائمة
حرفيًا، مرور النسبة **كنص** بلا تحويل، رفض السالب/غير الرقمي/تجاوز 3 منازل
قبل الوصول للـRPC، تمرير رسائل رفض القاعدة كما هي، وحالة تنقّل الشحنات.

**لم يُضَف أي اختبار SQL** — لم تتغيّر أي ترحيلة ولا أي دالة قاعدة، فالثوابت
الخلفية مُغطّاة أصلًا ولا يوجد ثابت جديد لإثباته.

**النتائج الفعلية:** Vitest **543/543** عبر 39 ملفًا · TypeScript **صفر
أخطاء** · ESLint **صفر أخطاء** (4 تحذيرات قائمة مسبقًا في ملف غير مرتبط) ·
بناء إنتاجي **54/54 صفحة** (كانت 53؛ المضاف `/master-data/vat-rates`) · فحص
تغطية SQL في CI: **صفر اختبارات يتيمة** (51 ملف SQL دون تغيير).

**لم تُشغَّل GitHub CI بعد — الفرع لم يُدفع.**

**خلاصة الملحق الثاني والأربعين:** إغلاق فجوة واجهة مُثبَتة كانت تجعل نسبة
ضريبة القيمة المضافة غير قابلة للتغيير إلا بـSQL مباشر، وتصحيح وسم تنقّل
قديم — بلا ترحيلات، وبلا مساس بـ`0001–0236` أو `seed.sql`، وبلا أي توسيع نطاق.

---

## الملحق الثالث والأربعون — "Phase 11: نواة المشتريات والموردين (Purchases & Suppliers Core)" (ترحيلات 0237–0240)

بعد موافقة صريحة على النطاق، نُفِّذت **Phase 11 = نواة المشتريات والموردين
فقط**: كتالوج موردين عام + دفتر فواتير شراء إضافي-فقط ببنود مُسعَّرة ضريبيًا +
دفتر مدفوعات موردين إضافي-فقط، مع **ترحيل مخزوني تلقائي ذرّي** عبر محرك
Phase 9 نفسه. وبلا أي وحدة مؤجَّلة: لا GL/COGS، لا ضريبة مدخلات قابلة
للاسترداد ولا إقرارات ضريبية، لا مرفقات، لا دورات اعتماد شرائية، لا تكامل
بنكي/Salla/ناقل، ولا مساس بحساب ربح المبيعات أو محرك تكلفة الإحلال.

**الفجوة التي أُغلقت:** لم يكن للنظام أي مفهوم لمورّد ولا لفاتورة شراء. كانت
الكميات تدخل المخزون بحركة `receive` يدوية بلا مستند مصدر، وبلا أي تسجيل
لالتزام مالي تجاه أحد — أي أن مستحقات الموردين كانت **خارج النظام بالكامل**.

### 1) الترحيلات الجديدة (0237–0240)

| # | الترحيلة | تُضيف |
|---|---|---|
| 0237 | `purchases_permissions.sql` | سبع صلاحيات (`purchases.view`/`create`/`reverse`/`record_payment`/`reverse_payment`/`manage_suppliers`/`process_closed_day`) ومنحها للأدوار. **مزروعة في الترحيلة نفسها لا في `seed.sql`**. توثيق صريح لسبب **عدم** تسجيل نطاق قفل استشاري جديد. |
| 0238 | `purchases_schema.sql` | `suppliers` (كتالوج عام، `row_version`، رمز دائم فريد بلا حساسية لحالة الأحرف)، `purchase_invoices` (دفتر إضافي-فقط موقَّع، لقطة اسم المورّد ورقمه الضريبي، فهرس فريد جزئي لرقم فاتورة المورّد لكل مورّد، وآخر لعكس واحد)، `purchase_invoice_lines` (معالجة ضريبية لكل بند + **رابط `NOT NULL UNIQUE` إلى حركة المخزون**)، `supplier_payments` (دفتر إضافي-فقط موقَّع). Triggers ترفض كل `UPDATE`/`DELETE` على الدفاتر الثلاثة؛ سياسات RLS طبقة-A. |
| 0239 | `purchases_rpcs.sql` | عشر دوال `SECURITY DEFINER` بـ`search_path` مثبَّت. كل قيمة مالية تُعاد `::text` **مقروءة من الصف المُدرَج**. |
| 0240 | `purchases_reads_and_reports.sql` | خمس دوال قراءة: كتالوج، دفتر المشتريات (محرك الشاشة **والتصدير** معًا، §39)، تفاصيل مستند، ملخص المستحق، وكشف حساب مورّد. |

**لم تُعدَّل أي ترحيلة من 0001 إلى 0236، ولا `supabase/seed.sql`.**

### 2) القرار الثاني — الترحيل المخزوني تلقائي وذرّي، وعبر محرك Phase 9 نفسه

ترحيل الفاتورة **هو** استلام البضاعة: لا انتظار لدفعة، ولا خطوة ثانية. وكل
كمية تمرّ عبر `record_inventory_stock_movement()` (0229) **بلا تعديل حرف واحد
فيها** — تُمرَّر إليها `'purchases.create'` كمعامل صلاحية (وهو بالضبط سبب
قبولها لمعامل صلاحية أصلًا)، فترث الشراء أقفالها (1008) وتحقّقاتها وتدقيقها.

الضمان البنيوي ضد الترحيل المزدوج أو الجزئي ليس اتفاقًا برمجيًا بل **قيد
قاعدة**: `purchase_invoice_lines.inventory_movement_id NOT NULL UNIQUE`. بند بلا
حركة مستحيل، وحركة تخدم بندين مستحيلة، والاثنان في معاملة واحدة.

العكس يُنشئ حركات تعويضية **بتاريخ عمل العكس الخاص** (§85) عبر المحرك نفسه —
بنوع `adjust` وكمية سالبة، لأن قيد 0228 يشترط أن يكون `receive` موجبًا و0229
يشترط سببًا لـ`adjust`.

### 3) القرار الأول — التزامات الموردين مشتقة، لا مخزَّنة

لا يوجد عمود «رصيد» في أي مكان. المتبقي دائمًا:

```
outstanding = gross_total − SUM(supplier_payments.amount)     ولا شيء إذا كانت الفاتورة معكوسة
```

المدفوعات إلحاق-فقط بمبلغ موقَّع، فعكس الدفعة يعيد الالتزام تلقائيًا بلا طرح
مزدوج. و`payment_status` (غير مسددة/جزئية/مسددة/معكوسة) **مشتقة في وقت
القراءة** من هذه الوقائع لا مقروءة من عمود.

**قاعدة الترتيب:** يُرفض عكس فاتورة ما دامت مرتبطة بدفعة **غير معكوسة** — وإلا
لبقي مال مدفوع منسوبًا إلى مستند لم يعد قائمًا.

### 4) القرار الرابع — بيانات ضريبية محفوظة بلا أي قرار استرداد

تُحفظ لقطة رقم المورّد الضريبي مع كل فاتورة (فتعديل بيانات المورّد لاحقًا لا
يعيد كتابة التاريخ)، ورقم وتاريخ فاتورة المورّد، و**المعالجة الضريبية والنسبة
والمبالغ لكل بند كما وردت بالضبط** — بلا إعادة احتساب وبلا تقريب. المعالجات
الأربع مدعومة (`standard`/`zero_rated`/`exempt`/`out_of_scope`)، ويُرفض بند
يناقض معالجته المعلنة (غير أساسي يحمل ضريبة أو نسبة).

التحقّق **تامّ لا تقريبي**: إجمالي كل بند = صافيه + ضريبته، وإجماليات الرأس =
مجموع البنود، وإلا رُفض المستند قبل كتابة أي شيء. و**الإجمالي هو ما يحدّد
الالتزام** تجاه المورّد.

**ما لا يفعله النظام صراحةً:** لا يحسب ضريبة مدخلات قابلة للاسترداد، ولا
يطالب بها، ولا يُصدر إقرارًا. لا يوجد عمود ولا حقل ملخّص ولا تسمية واجهة تلمّح
إلى الاستردادية — واختبار Vitest يفشل إن أُضيف أحدها. حقل `vat_total` مُسمّى في
الواجهة والتقرير «الضريبة كما وردت من الموردين»، وهو وصف لما جرى لا ادعاء
باستحقاق.

### 5) القرار الثالث — تكلفة الاقتناء توثيقية بحتة

`net_operating_return` وصيغ مصروفات Phase 10 وربح المبيعات ومحرك تكلفة الإحلال
والـGL والـCOGS: **لم يُمَسّ أيٌّ منها بسطر واحد**. المشتريات طبقة تقارير
منفصلة تمامًا، واختبار الترقية يُثبت ذلك بفحص مصدر الدوال نفسها بعد الترقية.

### 6) القرار الخامس — انفصال الدفترين، وإفصاح أمين عن الخطر المتبقي

المشتريات **لا تُنشئ صف `store_expenses` أبدًا**، وتقارير المصروفات لا تضمّ
المشتريات أبدًا. ومخطط مصروفات Phase 10 **لم يُعدَّل إطلاقًا** — تحديدًا لتجنّب
صنع ضمان دلالي زائف: عمود «ليس شراءً» في جدول المصروفات كان سيبدو حاجزًا وهو
ليس كذلك. الفصل قائم على انفصال الجداول والدوال، وهو مُثبَت في اختبار الترقية
(لا عمود، لا قيد، لا مفتاح أجنبي، ولا ذكر متبادل في مصادر الدوال).

**والخطر الذي لا يمكن منعه، مُصرَّح به:** يستطيع مستخدم أن يُدخل نفس فاتورة
الشراء يدويًا مرة أخرى في شاشة المصروفات. لا يوجد ربط يكشف ذلك، ولا يمكن
لتصنيف نصّي حر أن يوفّر ضمانًا بنيويًا ضده. المُتَّخَذ: **تحذير صريح في شاشتي
إدخال الفاتورة وقائمة المشتريات** ينصّ على أن شراء المخزون ليس مصروفًا تشغيليًا
وأن **النظام لا يمنع الازدواج تقنيًا** — ويوجد اختبار يفشل إن جرى تليين هذه
الجملة إلى وعد.

### 7) الصلاحيات

`super_admin` و`admin`: السبع. `supervisor`: الست التشغيلية **دون**
`manage_suppliers` — يطابق 0133 (يحجب `adjustments.manage_types`) و0233 (يحجب
`expenses.manage_categories`). `accountant`: **عرض فقط**. وتسجيل الدفعة وعكسها
صلاحيتان منفصلتان عن ترحيل الفاتورة وعكسها — أربع قدرات مختلفة لا قدرة واحدة.

### 8) الصفحات

`/purchases` (فلاتر بالتاريخ/الفرع/المورّد/الحالة/النوع/البحث، أربع بطاقات
إجمالية، تصدير PDF/Excel)، `/purchases/new` (نموذج متعدد البنود بإجماليات حيّة
بحساب سنتات صحيحة)، `/purchases/[id]` (المستند ببنوده ودفعاته، وعكس الفاتورة
والدفعة)، `/purchases/outstanding` (المستحق لكل مورّد + كشف حساب)، و
`/master-data/suppliers`. عنصر تنقل جديد مقيَّد بـ`purchases.view`، ومدخل في
صفحة البيانات الأساسية. تقرير `purchases` في سجل التصدير **يعيد استخدام نفس
محرك `list_purchase_invoices()`** الذي تستخدمه الشاشة (§39).

### 9) قرارات تصميم أساسية

1. **لا نطاق قفل استشاري جديد:** كل ثابت تزامن يغطيه ضمان أقوى أو قائم — قفل
   صف على الفاتورة (السباق على المتبقي، والدفعة مقابل العكس)، فهارس فريدة
   جزئية (رقم فاتورة المورّد، عكس واحد)، القفل 1008 القائم (المخزون)، والزوج
   1002 القائم (الإغلاق اليومي). القفل الصفّي هو الأداة الدقيقة هنا لأن المورد
   المتنازع عليه **هو الصف** فعلًا.
2. **`public.payment_methods` لم يُعَد استخدامه:** فهو يُنمذج **تحصيلًا واردًا**
   من عميل برسوم معالج وقنوات ومسارات تسوية — ولا شيء من ذلك ينطبق على دفعة
   صادرة لمورّد. أُضيف `payment_mode` بسيط بدل تحميل نموذج قائم دلالة ليست له.
3. **التحقّق قبل الكتابة:** تُتحقَّق كل البنود وتُجمع **قبل** إدراج الرأس، وإلا
   لظهر خطأ حسابي في بند على شكل اسم قيد `CHECK` خام بدل جملة تسمّي البند.
4. **الكتابة بنطاق الفروع القابلة للتشغيل، والقراءة بالمرئية** — وأي فلتر فرع
   خارج النطاق يُرفض لا يُضيَّق صامتًا (§8).

### 10) الاختبارات — نُفِّذت فعليًا مقابل PostgreSQL 16.4 حقيقي

| النوع | الملف | النتيجة |
|---|---|---|
| SQL أساسي | `purchases_core_phase11.test.sql` | **PASSED** |
| تزامن حقيقي | `purchases_phase11_concurrency.test.sql` | **PASSED — 10/10 تشغيلات متتالية** |
| ترقية | `upgrade_phase11_purchases.test.sql` + runner | **PASSED** من خط الأساس المجمَّد 0236 |
| Vitest | 4 ملفات جديدة (100 اختبارًا) | ضمن **643/643** |

اختبار التزامن يغطي الثوابت الستة المطلوبة، و**كلٌّ منها يُثبت التداخل الفعلي
عبر `pg_blocking_pids()`** قبل تحرير الجلسة الأولى — لا سيناريو يُثبَت بـ`pg_sleep`
وحده. سيناريو المخزون تحديدًا دليل مباشر على مرور الشراء عبر محرك Phase 9: لا
تشترك الفاتورتان في صف ولا مفتاح فريد، فلا شيء يمكن أن يحجب الثانية سوى القفل
1008 الذي يأخذه المحرك.

التفاصيل الكاملة في `TEST_RESULTS_PHASE_11_PURCHASES.md`.

### 11) مراجعة ما قبل الرفع — عيبان أُصلحا وثلاث فجوات تغطية سُدَّت

جرت مراجعة مركّزة على التثبيتة `0979099` قبل الرفع، وأسفرت عن تعديل مباشر داخل
`0239`/`0240` **غير المُعتمَدتين بعد** (نفس سابقة Hotfix 7.1.3 مع 0197–0198؛ لم
تُمَسّ `0001–0236` ولا `seed.sql`):

1. **عيب جمود حقيقي — ترتيب أقفال 1008 كان بيد العميل.** Phase 11 أول مستدعٍ
   يأخذ أكثر من قفل 1008 في معاملة واحدة (مرة لكل بند)، وكانت تُؤخذ بترتيب
   البنود كما أرسلها العميل. أُثبت بفحص `pg_locks` أن فاتورة محجوبة **تحمل
   بالفعل** قفل الصنف الآخر — الحافة الناقصة لدورة جمود بين فاتورتين
   متعددتَي البنود. أُصلح بالتقاط كل الأقفال مقدَّمًا **بترتيب قانوني** في
   مساري الترحيل والعكس، دون أي مساس بمحرك Phase 9. **مُثبَت بالطفرة:** إزالة
   الحلقة تُفشل السيناريو G برسالته الحرفية.
2. **الحارس اليومي كان مُختبَرًا في موضع واحد من أربعة.** أُضيفت تغطية الدفع
   وعكس الفاتورة وعكس الدفعة (وظيفيًا)، والإغلاق مقابل **الدفع** (تزامنيًا،
   السيناريو H).
3. **الذرّية كانت مُستنتَجة لا مُختبَرة.** أُضيف تطابق بند-بحركة حقلًا بحقل
   (صنف/فرع/كمية موقَّعة/اتجاه) لكل بنود القاعدة، واختبار فشل منتصف المعاملة
   يثبت أن بندًا فاشلًا بعد بند ناجح **لا يترك فاتورة ولا بندًا ولا حركة مخزون
   واحدة**، مع حارس بنيوي يمنع الاختبار من المرور دون إنشاء حالة جزئية أصلًا.
4. **دلالة التاريخ صارت صريحة.** أُضيف إلى كشف حساب المورّد رصيدٌ افتتاحي
   ورصيدٌ **حتى تاريخ التقرير** ورصيدٌ **حالي**، معروضة منفصلة وموسومة بتواريخها
   مع تنبيه عند اختلافها؛ وأُعيدت تسمية حقول تقرير المشتريات لتقول «حاليًا»
   صراحةً. والثابت المُثبَت: عكس مؤرَّخ لاحقًا **لا يغيّر** أي رقم في كشف فترة
   منتهية.
5. **التصدير الفعلي صار مُغطّى** بملف Vitest جديد (نطاق الفرع حرفيًا، رفض الفرع
   خارج النطاق يُرمى بلا تسريب إجماليات، وتكافؤ الشاشة والتصدير).

النتائج بعد الإصلاح: الاختبارات الثلاثة لـPhase 11 **ناجحة على قواعد نظيفة**،
والتزامن **8 سيناريوهات × 10/10 تشغيلات**، وVitest للمشتريات **113/113**،
وtsc/ESLint/build نظيفة.

**خلاصة الملحق الثالث والأربعين:** دفتر مشتريات ومستحقات موردين جديد كليًا،
مربوط بالمخزون عبر محرك Phase 9 القائم بترحيل ذرّي مضمون بقيد قاعدة، ومنفصل
تمامًا عن دفتر المصروفات التشغيلية وعن كل صيغة ربحية قائمة. البيانات الضريبية
محفوظة كما وردت **بلا أي قرار استردادية**. لا تعديل على أي ترحيلة من 0001–0236
ولا على `seed.sql`، ولا أي وحدة خارج النطاق المعتمد. **نتائج CI على GitHub غير
متاحة بعد — لم يُدفع الفرع.**
