-- ============================================================================
-- 0095: Returns Integrity Patch 4.1 (4/7): approve_sales_return(),
-- reject_sales_return()
-- ============================================================================
-- Migrations 0001-0094 are unmodified.
--
-- approve_sales_return() is rewritten for Sections 1/2/4/5/6/12/17/19:
--  * Global safe lock order (Section 19) — sales_returns row, THEN
--    sales_orders row FOR UPDATE, THEN the returns advisory lock, THEN
--    sales_order_items rows FOR UPDATE ORDER BY id. update_sales_order()
--    (0084, unmodified) already locks sales_orders row BEFORE the advisory
--    lock; matching that exact order here (instead of the reverse) is what
--    prevents the two-transaction deadlock Section 19 warns about — see
--    0111's dedicated concurrency test for a real proof no deadlock occurs.
--  * Stale-sale guard (Section 4) — rejects if the Sale's CURRENT
--    row_version no longer matches source_sale_row_version, or if any
--    referenced item is no longer active/no longer belongs to this Sale.
--    Never silently re-snapshots — refresh_pending_sales_return_from_sale()
--    (0093) is the only path to a fresh basis.
--  * Effective-claim exclusivity (Section 5) — an explicit pre-check plus
--    sales_return_items_effective_claim_uq (0092) as the real DB backstop;
--    concurrent approvals of two pending returns on the same item are
--    already serialized by the per-order advisory lock (both returns share
--    one sales_order_id), so the second call observes the first's
--    committed is_effective=true and is rejected with the exact spec
--    message; the unique index remains the last-resort guarantee.
--  * The full Section 12 financial-effect computation, written ONLY here:
--    returned_original_sale_amount, sales_revenue_reversal_amount (=
--    returned_original_sale_amount - non_shipping_deduction_amount, using
--    the deduction ALREADY stored on the return from create/update-pending,
--    Section 16 — "approval recomputes and re-validates", it does not
--    accept new deduction/refund inputs of its own), recovered_original_
--    cost_amount, payment_fee_reversal_amount (Section 8 formula, 0092),
--    net_sales_profit_adjustment. approved_refund_amount is NOT recomputed
--    here (Section 1 — it is an explicit business input, never derived) but
--    IS re-validated against the freshly-computed revenue reversal.
--    gross_profit_reversal_amount/net_profit_reversal_amount (the pre-Patch
--    columns) are kept and still written, documented as compatibility
--    figures — net_sales_profit_adjustment is the authoritative one.
...
comment on function public.approve_sales_return(uuid, bigint, numeric, text) is
  'Patch 4.1 (Sections 1/2/4/5/6/12/17/19) — same signature as 0087, entirely rewritten body. Global safe lock order: sales_returns row -> sales_orders row FOR UPDATE -> returns advisory lock -> sales_order_items rows FOR UPDATE ORDER BY id (matches update_sales_order()''s own order, 0084 — no deadlock possible between the two). Rejects if the Sale''s row_version no longer matches source_sale_row_version, or any item is no longer active/no longer belongs to the Sale (Section 4) — never silently re-snapshots. Computes returned_original_sale_amount/sales_revenue_reversal_amount/recovered_original_cost_amount/net_sales_profit_adjustment (Section 12) and re-validates approved_refund_amount/refund_difference_reason/customer_never_received against the fresh totals. Flips is_effective=true (+included_in_decision=true) for the return''s active items — sales_return_items_effective_claim_uq (0092) is the real concurrency backstop. SECURITY DEFINER.';
...
-- ---------------------------------------------------------------------------
-- reject_sales_return() — Section 6 fix: no longer cascades sales_
-- return_items to status='removed' (that erased historical item visibility
-- for good). Only included_in_decision is set, marking exactly which items
-- were part of the request at the moment of rejection — get_sales_return()
-- (0098) shows them forever after via status='active' OR included_in_
-- decision=true. No financial columns are ever written for a rejected
-- return (unchanged — a rejection never went through approval). Section 13
-- — store scope relaxed from OPERABLE to VISIBLE (historical correction on
-- an already-created Pending return, not a brand-new transaction; a
-- processing store disabled after the return was created must not freeze
...
begin
  ...
  -- Section 6 — mark the final requested item set, never erase it.
  update public.sales_return_items
...
comment on function public.reject_sales_return(uuid, bigint, text) is
  'Patch 4.1 (Sections 6/13) — no longer cascades sales_return_items to status=''removed'' (that erased historical item visibility). Only included_in_decision is set on the items active at the moment of rejection; get_sales_return() (0098) shows them forever after. Store scope relaxed to VISIBLE (Section 13 — a historical correction on an already-open Pending return, not a new transaction; must not be blocked by the processing store later being disabled). SECURITY DEFINER.';
