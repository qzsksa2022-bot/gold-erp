-- ============================================================================
-- 0098: Returns Integrity Patch 4.1 (7/7): list_sales_returns(),
-- get_sales_return(), get_returnable_sales_order()
-- ============================================================================
-- Migrations 0001-0097 are unmodified. Same access-model reasoning as 0090:
-- zero direct-authenticated RLS policies on any Returns table, so these
-- three SECURITY DEFINER RPCs remain the only read path.
--
-- Section 14 — list_sales_returns() gains real filters (original store,
-- order number, scenario — date/processing-store/return-number/status/
-- sales_order_id already existed) and the full Section 12 financial-effect
-- + Section 11 reconciliation-state display fields.
--
-- Section 6 — item visibility (both here and in get_sales_return()) is now
-- `status = 'active' OR included_in_decision = true`, so a Rejected or
-- Reversed return's item_count/item list never reads as empty just because
-- the effective claim was released or the return was rejected.
...
begin
  ...
  select
    ...
    (coalesce(sr.approved_refund_amount, 0) - (select coalesce(sum(e.amount), 0.00) from public.sales_return_refund_events e where e.sales_return_id = sr.id and e.status = 'active'))::text,
    case
...
comment on function public.list_sales_returns(date, date, uuid, uuid, text, text, text, text, uuid, integer, integer) is
  'Patch 4.1 (Section 14) — adds original_store_id/order_number/scenario filters, original-sale-date/collection_state/returned_original_sale_amount/deduction/actual_refund/refund_variance/refund_reconciliation_state, and (sales.view_profit) recovered cost/fee reversal/net_sales_profit_adjustment/adjusted_order_net_sales_profit. item_count uses status=''active'' OR included_in_decision=true (Section 6) — never reads as zero for a Rejected/Reversed return. Scoped by processed_store_id via user_visible_store_ids().';
...
begin
  ...
  -- Reversed return never appears to have zero items.
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'refund_business_date', e.refund_business_date, 'refunded_at', e.refunded_at, 'notes', e.notes, 'status', e.status,
      'reversed_at', e.reversed_at, 'reversal_business_date', e.reversal_business_date, 'reversal_reason', e.reversal_reason
    )
...
comment on function public.get_sales_return(uuid) is
  'Patch 4.1 (Sections 6/12/14) — items visible if status=''active'' OR included_in_decision=true, so a Rejected/Reversed return never shows zero items. Adds collection_state/source_sale_row_version/returned_original_sale_amount/deduction/refund_difference_reason/refund_reconciliation_state/reversal_business_date and (sales.view_profit) recovered cost/payment fee reversal/net_sales_profit_adjustment/adjusted_order_net_sales_profit. Profit-sensitive keys entirely absent without sales.view_profit.';
...
-- ---------------------------------------------------------------------------
-- get_returnable_sales_order() — `returnable`/order_state now derive from
-- is_effective (Section 5), not mere `status='active'` membership: an item
-- referenced only by another still-PENDING return remains returnable (a
-- second pending return CAN target it); only an EFFECTIVE (approved) claim
-- makes it unreturnable. Also now returns the Sale's row_version, which the
-- New Return form must send back as p_expected_sale_version (Section 4).
-- ---------------------------------------------------------------------------
...
comment on function public.get_returnable_sales_order(uuid) is
  'Patch 4.1 (Sections 4/5) — `returnable`/order_state now derive from is_effective, not mere active membership: an item referenced only by another still-pending return remains returnable. Now also returns the Sale''s row_version, which the New Return form must round-trip as create_sales_return()''s p_expected_sale_version (Section 4). Requires sales.view AND returns.create. Profit-sensitive gross_profit gated on sales.view_profit.';
