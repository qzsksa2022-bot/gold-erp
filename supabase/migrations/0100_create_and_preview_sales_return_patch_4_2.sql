-- ============================================================================
-- 0100: Final Returns Integrity Patch 4.2 (2/7): create_sales_return(),
-- preview_sales_return(), refresh_pending_sales_return_from_sale()
-- ============================================================================
-- Migrations 0001-0099 are unmodified.
--
-- create_sales_return() — ONLY change from 0093: explicitly inserts
-- requires_sale_refresh=false (Section 1) — a brand-new return always
-- captures its snapshot and source_sale_row_version together, atomically,
-- in the same statement, so there is no window for them to diverge. Every
-- other line of body/validation/lock-order logic is byte-for-byte
-- unchanged from 0093.
--
-- refresh_pending_sales_return_from_sale() — ONLY change from 0093: also
-- clears requires_sale_refresh=false (Section 1) — this IS the one explicit
-- refresh path Section 1 requires; after it runs, the return's snapshots
-- are freshly re-taken from the CURRENT Sale, so the invariant genuinely
-- holds again.
--
-- preview_sales_return() — Section 5 (parity fix): previously accepted most
-- Business Inputs but silently ignored scenario entirely, never applied the
-- customer_never_received rule, and always suggested suggested_approved_
-- refund_amount = revenue_reversal regardless of scenario — so a UI that
-- (correctly) forced approved_refund_amount=0 for customer_never_received +
-- not_collected could see Preview "suggest" a nonzero figure right after,
-- and any invalid combination (deduction exceeding the total, an unexplained
-- refund difference) was accepted silently instead of raising the exact
-- same rejection create_sales_return()/update_pending_sales_return() would.
-- Now: gains p_scenario (new positional parameter — old 8-arg overload is
-- DROPped, not kept, since a stale ambiguous overload actively hides this
-- bug's fix); applies the identical customer_never_received suggestion rule
-- Section 2 defines; and re-runs the SAME validation checks create_sales_
-- return() applies (deduction bound/reason, refund-difference-reason,
-- money-scale) WHENEVER the corresponding input was actually supplied —
-- never invents a value the caller did not provide, never silently
-- substitutes its own suggestion for an explicit p_approved_refund_amount.
-- Still read-only, still not Source of Truth — approval recomputes fully.
...
begin
  ...
  insert into public.sales_returns (
    ...
    sale_date_snapshot, source_sale_row_version, requires_sale_refresh,
    ...
    v_order.sale_date, v_order.row_version, false,
...
comment on function public.create_sales_return(uuid, uuid, date, text, jsonb, bigint, text, numeric, numeric, text, text, text, text) is
  'Patch 4.1/4.2 — p_items is jsonb (per-item condition/reason/notes, Section 3). Requires p_expected_sale_version (Section 4), p_collection_state, p_approved_refund_amount, plus optional deduction/refund-difference fields (Section 1). return_date must be >= the Sale''s sale_date (Section 7). Double-return pre-check blocks only on an EFFECTIVE claim (Section 5). Patch 4.2 (0100): always inserts requires_sale_refresh=false — snapshot and source_sale_row_version are captured together, atomically, so there is no divergence window at creation. SECURITY DEFINER.';
...
-- ---------------------------------------------------------------------------
-- preview_sales_return() — Section 5 parity fix. New p_scenario parameter
-- inserted right after p_items; the old 8-arg (no p_scenario) overload is
-- explicitly DROPped below so nothing can keep calling the pre-fix version.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Section 2 suggestion rule — the actual fix: customer_never_received +
  -- not_collected always suggests 0, exactly matching the hard DB CHECK
  -- constraint (0092) and approve_sales_return()''s own re-validation
  -- (0101); every other combination suggests the full revenue reversal, as
  -- before. Never overrides an explicitly-supplied p_approved_refund_amount
  -- in the response — that is echoed back as-is (with its own variance),
  -- the caller decides what to do with the suggestion.
...
comment on function public.preview_sales_return(uuid, jsonb, text, text, numeric, text, numeric, text, numeric) is
  'Patch 4.2 (Section 5) — gains p_scenario (new positional parameter — old 8-arg overload dropped). suggested_approved_refund_amount is now 0 for customer_never_received + not_collected (matching the real DB CHECK constraint and approve_sales_return()), the full revenue reversal otherwise. Re-runs create_sales_return()''s own validation tree (deduction bound/reason, refund-difference-reason, money-scale, customer_never_received) whenever the corresponding input is actually supplied — raises the exact same error create would, never silently substitutes a different value for an explicit approved_refund_amount. Still an ESTIMATE, still not Source of Truth. Decimal-safe transport. Profit-sensitive keys absent without sales.view_profit.';

drop function if exists public.preview_sales_return(uuid, jsonb, text, numeric, text, numeric, text, numeric);
...
-- ---------------------------------------------------------------------------
-- refresh_pending_sales_return_from_sale() — ONLY change from 0093: also
-- clears requires_sale_refresh (Section 1) — this call IS the one explicit
-- refresh the flag demands; every other line is unchanged.
-- ---------------------------------------------------------------------------
...
begin
  ...
  perform public.log_audit_event(
    ...
    jsonb_build_object('source_sale_row_version', v_old_return.source_sale_row_version, 'row_version', v_old_return.row_version, 'requires_sale_refresh', v_old_return.requires_sale_refresh),
    jsonb_build_object('source_sale_row_version', v_order.row_version, 'row_version', v_new_row_version, 'requires_sale_refresh', false, 'refreshed_from_sale', true)
  );
...
comment on function public.refresh_pending_sales_return_from_sale(uuid, bigint) is
  'Patch 4.1/4.2 (Section 4/1) — the ONLY way a Pending return''s Sale-derived snapshots (header + every active item) are ever re-taken after creation, and the ONLY way requires_sale_refresh (0099) is ever cleared. approve_sales_return() (0101) NEVER does either implicitly — a stale or unrefreshed basis is rejected outright. Does not change the item SET. SECURITY DEFINER.';
