-- ============================================================================
-- 0104: Final Returns Integrity Patch 4.2 (6/7): get_sales_return(),
-- list_sales_returns()
-- ============================================================================
-- Migrations 0001-0103 are unmodified. Same signatures/access model as
-- 0098 — both gain new read-only fields, nothing about visibility scope,
-- profit-gating shape, or item-history logic changes.
--
-- Section 1 — requires_sale_refresh (0099/0100) is now exposed on both, so
-- the UI can show a "يجب تحديث بيانات المرتجع من عملية البيع" banner and
-- offer refresh_pending_sales_return_from_sale() BEFORE the user even
-- attempts (and is rejected by) approve_sales_return() (0101). Not profit-
-- sensitive — visible to any returns.view caller, exactly like status/
-- collection_state.
--
-- Section 4 — get_sales_return() gains reconciliation_history: every
-- sales_return_refund_reconciliation_events (0099) row for this return, in
-- order, written by finalize_sales_return_refund()/reopen_sales_return_
-- refund_reconciliation() (0103). Not profit-gated — mirrors refund_events/
-- actual_refunded_total/refund_variance/approved_refund_amount, which are
-- already visible to any returns.view caller without sales.view_profit.
...
comment on function public.get_sales_return(uuid) is
  'Patch 4.1/4.2 (Sections 1/4/6/12/14) — items visible if status=''active'' OR included_in_decision=true. Patch 4.2 (0104): adds requires_sale_refresh (not profit-gated — Section 1, so the UI can offer refresh BEFORE approval is attempted and rejected) and reconciliation_history (full finalize/reopen transition history from sales_return_refund_reconciliation_events, 0099/0103, not profit-gated). Profit-sensitive keys entirely absent without sales.view_profit.';
...
-- ---------------------------------------------------------------------------
-- list_sales_returns() — adds requires_sale_refresh to the row shape
-- (Section 1), otherwise byte-for-byte unchanged from 0098.
-- ---------------------------------------------------------------------------
...
comment on function public.list_sales_returns(date, date, uuid, uuid, text, text, text, text, uuid, integer, integer) is
  'Patch 4.1/4.2 (Sections 1/14) — adds requires_sale_refresh (0104) to the row shape, otherwise unchanged from 0098. item_count uses status=''active'' OR included_in_decision=true (Section 6). Scoped by processed_store_id via user_visible_store_ids().';
