-- ============================================================================
-- 0159: Phase 6 Final Integrity Hotfix 6.1.1 (3/6): approve_sales_order_
-- adjustment() v3 — writes calculation_version authoritatively
-- ============================================================================
-- Migrations 0001-0158 are unmodified. Same 3-argument signature and return
-- shape as 0148 — CREATE OR REPLACE in place, no drop needed.
--
-- Hotfix 6.1.1 item 7 — the ONLY place calculation_version (0158) is ever
-- written: every successful approval stamps calculation_version = 1 (the
-- single calculation contract that has existed since approval was first
-- introduced). Never a client-supplied parameter — no p_calculation_version
-- argument exists or ever will on this or any other RPC; the 0158 CHECK
-- constraint (approved <=> calculation_version is not null) is the backstop
-- against any future path forgetting to set it.
-- ---------------------------------------------------------------------------
...
begin
  ...
  perform public.log_audit_event(
    ...
    jsonb_build_object(
      ...
      'is_free_service', v_is_free, 'calculation_version', 1, 'row_version', v_new_row_version
    )
  );
...
comment on function public.approve_sales_order_adjustment(uuid, bigint, text) is
  'Phase 6 (§17) + Patch 6.1 items 5/6/8/9/14 + Hotfix 6.1.1 item 7 — full approval procedure. Requires adjustments.approve ALONE. direct_cost IS NULL still hard-rejects. Recomputes payment_fee/gross/net authoritatively; a customer_charge=0 record skips fee resolution entirely. adjustment_date is revalidated against the Sale''s sale_date. net_adjustment_profit in the response is NULL without sales.view_profit. Stamps calculation_version=1 authoritatively (0158) — never a client input. SECURITY DEFINER.';
