-- ============================================================================
-- 0161: Phase 6 Final Integrity Hotfix 6.1.1 (5/6): update_sales_order_
-- adjustment() v3 — full old/new audit payload
-- ============================================================================
-- Migrations 0001-0160 are unmodified. Same 12-argument signature and
-- return shape as 0147 — CREATE OR REPLACE in place, no drop needed.
--
-- Hotfix 6.1.1 item 8 — the general system requirement is old/new/who/when/
-- reason on every mutation's audit row (log_audit_event already always
-- captures who/when via the trigger-free actor/timestamp columns on
-- audit_logs itself). adjustment.update's payload only ever recorded
-- customer_charge/row_version, silently omitting every OTHER field this RPC
-- can change (adjustment_type_id/processing_store_id/adjustment_date/
-- payment_method_id/collection_channel_id/payment_reference/participates_
-- in_settlement/notes). This migration expands the old_values/new_values
-- payload to cover every field the RPC actually mutates. set_pending_
-- sales_order_adjustment_direct_cost() (0145) already logs old/new
-- direct_cost correctly as its own dedicated adjustment.cost_set action —
-- untouched here, already correct. No new audit action is introduced;
-- adjustment.update stays gated by sales.view_profit exactly as before
-- (0143/0156) — customer_charge is the only money figure in this payload,
-- and it was ALREADY there pre-hotfix, so the gate's effective scope is
-- unchanged.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Hotfix 6.1.1 item 8 — full old/new payload across every field this RPC
  -- can actually change (sales_order_id/id are immutable and never appear
  -- here since they can never change). Still just adjustment.update — no
  -- new audit action, same sales.view_profit gate as before (0143/0156).
  perform public.log_audit_event(
    'adjustment.update', 'sales_order_adjustment', p_id,
    jsonb_build_object(
      'adjustment_type_id', v_old.adjustment_type_id,
      'processing_store_id', v_old.processing_store_id,
      'adjustment_date', v_old.adjustment_date,
      'payment_method_id', v_old.payment_method_id,
      'collection_channel_id', v_old.collection_channel_id,
      'payment_reference', v_old.payment_reference,
      'participates_in_settlement', v_old.participates_in_settlement,
      'customer_charge', v_old.customer_charge,
      'notes', v_old.notes,
      'row_version', v_old.row_version
    ),
    jsonb_build_object(
      'adjustment_type_id', p_adjustment_type_id,
      'processing_store_id', p_processing_store_id,
      'adjustment_date', p_adjustment_date,
      'payment_method_id', v_payment_method_id,
      'collection_channel_id', v_collection_channel_id,
...
comment on function public.update_sales_order_adjustment(uuid, bigint, uuid, uuid, date, uuid, uuid, boolean, numeric, text, text, text) is
  'Phase 6 (§16) + Patch 6.1 items 2/7/8/9/10/11/14 + Hotfix 6.1.1 item 8 — edits a PENDING Service/Adjustment. direct_cost is NEVER accepted here. audit payload (adjustment.update) now covers every field this RPC can actually change (type/store/date/payment method/channel/reference/settlement/charge/notes), not customer_charge alone. Requires adjustments.create. SECURITY DEFINER.';
